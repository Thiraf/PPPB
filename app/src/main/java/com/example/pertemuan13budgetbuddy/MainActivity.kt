...
import com.example.pertemuan13budgetbuddy.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        with(binding){
            ...
            listView.setOnItemClickListener{
                adaterView, _, i, _->
                val item = adaterView.adapter.getItem(i) as Budget
                updateId = item.id
    ...
    private fun getAllBudgets(){
        budgetCollectionRef.addSnapshotListener { snapshots, error ->
            if (error != null) {
                Log.d(
                    "MainActivity", "Error Listening for budget changes",
                    error
                )
            ...
            snapshots?.forEach { documentRefrence ->
                budgets.add(
                    Budget(
                        documentRefrence.id,
                        documentRefrence.get("nominal").toString(),
                        documentRefrence.get("description").toString(),
                        documentRefrence.get("date").toString()
                    )
                )
            }
            if (budgets != null) {
